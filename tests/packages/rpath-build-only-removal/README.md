# rpath-build-only-removal

Remove all build-only entries without creating an empty dynamic path.

## Layout and inputs

Build: _probe with build_rpath.

Wheel: _probe → no project libraries; installed paths empty.

Run this package alone from the meson-python checkout:

```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_build_only_removal' -vv
```
...
| --- | --- | --- |
| `probe/_probe*` | No project paths | No project paths |
