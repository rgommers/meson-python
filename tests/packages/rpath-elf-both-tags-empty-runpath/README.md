# rpath-elf-both-tags-empty-runpath

An empty DT_RUNPATH must still suppress DT_RPATH.
...
DT_RUNPATH is present but empty.
...
| --- | --- | --- |
| `probe/probe-exe` | Empty DT_RUNPATH (the tag must exist); DT_RPATH may retain only `$ORIGIN/rpath-choice`, or be removed | Not applicable |
...
The executable must fail to load `librpath_test_choice.so`, although that
library exists under the ignored RPATH. `smoke.py` requires the nonzero exit
status and the specific missing-library diagnostic; this is an intentional
negative loader test.
...
```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_elf_both_tags_empty_runpath' -vv
```
