# rpath-elf-install-precedence

Explicit installation paths must precede preserved paths: same SONAME, different answers.

## Layout and inputs

Build: probe-exe links private/libchoice; external/libchoice has the same SONAME.

Wheel: probe/probe-exe must select probe/private/libchoice (42), not probe/external/libchoice (7).

Run this package alone from the meson-python checkout:

```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_elf_install_precedence' -vv
```
...
| --- | --- | --- |
| `probe/probe-exe` | `$ORIGIN/private`, `$ORIGIN/external` | Not applicable |
| `probe/external/*choice*` | No project paths | Not applicable |
| `probe/private/*choice*` | No project paths | Not applicable |
