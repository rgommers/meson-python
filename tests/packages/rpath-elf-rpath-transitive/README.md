# rpath-elf-rpath-transitive

Preserve DT_RPATH transitive loader semantics.

## Layout and inputs

Build: probe-exe → middle/libmiddle → leaf/libleaf.

Wheel: probe/probe-exe DT_RPATH → probe/lib/libmiddle → probe/lib/libleaf; middle has no project RPATH.

Run this package alone from the meson-python checkout:

```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_elf_rpath_transitive' -vv
```
...
| `probe/lib/*middle*` | No project paths | Not applicable |
