# rpath-in-package-nested

Each edge in an in-package dependency chain needs its own relative path.

## Layout and inputs

Build: module.c → first/libfirst → second/libsecond.

Wheel: probe/_probe → probe/libfirst → probe/sub/libsecond.

Run this package alone from the meson-python checkout:

```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_in_package_nested' -vv
```
...
| `**/*first*` | `$ORIGIN/sub` | `@loader_path/sub` |
