// SPDX-FileCopyrightText: 2026 The meson-python developers
//
// SPDX-License-Identifier: MIT

#include <Python.h>

extern int first(void);
extern int second(void);
extern int private_value(void);

static PyObject *value(PyObject *self, PyObject *args) {
    return PyLong_FromLong(first() + second() + private_value());
}

static PyMethodDef methods[] = {
    {"value", value, METH_NOARGS, NULL},
    {NULL, NULL, 0, NULL},
};

static struct PyModuleDef module = {
    PyModuleDef_HEAD_INIT, "_mixed", NULL, -1, methods,
};

PyMODINIT_FUNC PyInit__mixed(void) { return PyModule_Create(&module); }
