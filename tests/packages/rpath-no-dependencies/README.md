# rpath-no-dependencies

No project dependency must not introduce a relocation RPATH.

## Layout and inputs

Build: _probe.

Wheel: _probe → no project libraries.

Run this package alone from the meson-python checkout:

```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_no_dependencies' -vv
```
