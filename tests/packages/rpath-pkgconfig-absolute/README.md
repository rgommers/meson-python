# rpath-pkgconfig-absolute

Preserve the absolute external RPATH supplied by pkg-config; use an absolute library filename like scipy-openblas64.

## Layout and inputs

Build: provider → external prefix; .pc Libs: /absolute/libexternal + RPATH.
...
```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_pkgconfig_absolute' -vv
```
