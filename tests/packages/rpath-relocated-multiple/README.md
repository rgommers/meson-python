# rpath-relocated-multiple

Two distinct build-library paths must become one relocation path (issue #813).

## Layout and inputs

Build: module.c → first/libfirst + second/libsecond.

Wheel: probe/_probe → two siblings in .rpath_relocated_multiple.mesonpy.libs.

Run this package alone from the meson-python checkout:

```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_relocated_multiple' -vv
```
...
| --- | --- | --- |
| `probe/_probe*` | `$ORIGIN/../.rpath_relocated_multiple.mesonpy.libs` | `@loader_path/../.rpath_relocated_multiple.mesonpy.libs` |
