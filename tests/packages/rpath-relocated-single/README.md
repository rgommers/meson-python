# rpath-relocated-single

One relocated project library needs exactly one .mesonpy.libs path.

## Layout and inputs

Build: module.c → first/libfirst.

Wheel: probe/_probe → .rpath_relocated_single.mesonpy.libs/libfirst.

Run this package alone from the meson-python checkout:

```sh
python -m pytest tests/test_rpath_packages.py -k 'rpath_relocated_single' -vv
```
...
| --- | --- | --- |
| `probe/_probe*` | `$ORIGIN/../.rpath_relocated_single.mesonpy.libs` | `@loader_path/../.rpath_relocated_single.mesonpy.libs` |
